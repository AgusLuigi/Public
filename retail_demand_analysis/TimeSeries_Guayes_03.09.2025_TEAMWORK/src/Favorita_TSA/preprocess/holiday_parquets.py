...
def save_holiday_enriched_tables(scope_days: int = 5) -> None:
    ...
    df_store_item_daily = pd.read_parquet(
        _preprocessed_path(PreDataset.STORE_ITEM_DAILY)
    )
    df_store_item_weekly = pd.read_parquet(
        _preprocessed_path(PreDataset.STORE_ITEM_WEEKLY)
    )

    # ---------------------------------------------
    # Build calendars with scope
    # ---------------------------------------------
    store_cal_daily = build_store_calendar_daily(
        df_stores=df_stores,
        df_holidays_events=df_holidays,
        cfg=cfg,
        scope_days=scope_days,
    )

    store_cal_weekly = build_store_calendar_weekly_from_daily(
        store_calendar_daily=store_cal_daily,
        cfg=cfg,
    ...
    # Enrich tables
    # ---------------------------------------------
    daily_out = enrich_store_item_daily(
        df_store_item_daily,
        store_cal_daily,
    )

    weekly_out = enrich_store_item_weekly(
        df_store_item_weekly,
        store_cal_weekly,
        week_start_col="week_start",
    )
    ...
    _save(
        daily_out,
        "data/processed/preprocessed/store_item_daily_holiday.parquet",
    )

    _save(
        weekly_out,
        "data/processed/preprocessed/store_item_weekly_holiday.parquet",
    )
